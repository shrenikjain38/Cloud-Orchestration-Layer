...
def vmCreate():
    ...
    for i in pm_file:
        try:
            user = pm_file[i].split("@")[0]
            print user
            print vm_image_path
            connection = libvirt.open("qemu+ssh://"+ pm_file[i]+"/system")
            path1="scp "+vm_image_path+pm_file[i]+":/home/"+user
            print path1
            os.system(path1)
            conn_created = 1
            vm_pm = i
    ...
    tmp = {}
    x = vm_image_path.split("/")
    po = x[len(x)-1]
    # print "Chal h"+po
    try:
        increment()
        xml = fd.read()% (str(present_vm_id),vm_name,vm_ram,vm_ram,vm_vcpu,"/home/"+user+"/"+po)
...
def vmDestroy():
    ...
    print pm_file[pmid]
    print name
    conn = libvirt.open("qemu+ssh://"+ pm_file[pmid]+"/system")
